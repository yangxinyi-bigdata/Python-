<template>
  <div class="flex h-screen bg-gray-100">
    ...
    <div class="flex-1 p-8">
      <h1 class="text-2xl font-semibold mb-6">中译英V2</h1>
...
const startTranslation = () => {
  ...
  // Determine which translation function to use and set appropriate params
  switch (translationMode.value) {
    case '原项目中翻译':
      translationFunction = http.开始翻译_中文原项目
      params = { 项目路径: projectPath.value }
      break
    case '翻译到新项目':
      translationFunction = http.开始翻译_中文项目
      params = { 项目路径: projectPath.value, 翻译路径: translationPath.value }
      break
    case '单个py / jupyter代码':
      translationFunction = http.开始翻译_中文py文件
      params = { 文件路径: projectPath.value, 翻译路径: translationPath.value }
      break
    default:
  ...
  // Call the appropriate translation function with its specific params
  translationFunction(params)
    .then(() => {
